...
@interface RCTXMLReader () <NSXMLParserDelegate>
...
+ (void)convertXMLData:(NSData *)data completion:(OutputBlock)completion
{
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    [[RCTXMLReader new] parser:parser completion:completion];
  });
}
...
- (void)parser:(NSXMLParser *)parser parseErrorOccurred:(NSError *)parseError
{
  ...
  NSLog(@"Line:%li Column:%ld - Parse Error Occurred: %@", (long)[parser lineNumber], [parser columnNumber], [parseError description]);
...
- (void)parser:(NSXMLParser *)parser validationErrorOccurred:(NSError *)validationError
{
  ...
  //Output in the console error.
  NSLog(@"Line:%li Column:%ld - Validation Error Occurred: %@", (long)[parser lineNumber], [parser columnNumber], [validationError description]);
